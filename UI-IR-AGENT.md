...
- 可选：可用本地资源路径列表（如 `../../__assets__/xxx.png`），引用切图时必须逐字复制列表中的路径
...
8. **重叠与装饰**：当某元素与上方区块有视觉重叠（如用户卡压在蓝色顶栏下方）时，须在该元素 `data-notes` 中写明，例如：`overlaps header_bg, implement with negative marginTop`。当某区域有**波浪、弧线、渐变边**等装饰时，须在对应元素的 `data-notes` 中说明（如：`底部波浪/弧线，实现可用大圆角、CSS 渐变、伪元素或普通占位图块，禁止 SVG`），避免实现时被简化为纯色块而丢失设计。
...
13. **【关键】禁止 SVG**：不得输出 `<svg>`、`<path>`、`<circle>`、`<rect>`、`<defs>`、`<g>`、`<use>` 等 SVG 标签，也不得内联 SVG 图标。切图覆盖不到的区域用普通 `div/span/img` 和 CSS 占位图块表达。
14. **【关键】保持可选中**：不得输出 `pointer-events: none`、`user-select: none` 以及 `-webkit/-moz/-ms-user-select: none`。所有可见元素都应能被预览区元素选择器命中。
15. **【关键】资源路径不得编造**：引用已有切图时，只能逐字复制调用方提供的可用本地资源路径；禁止按页面名、图层名或设计稿文件名自行拼接、改名、补全资源文件名。若没有可用切图，用普通 HTML/CSS 占位图块，不要引用不存在的图片。
16. **【关键】viewport**：`meta viewport` 必须使用 `width=device-width, initial-scale=1`。页面根容器按设计图实际宽度设置 `max-width`，不要写 `width=375` 或固定 375px 根容器。
...
- 圆角、阴影、描边尽量估算并写入。
- 大面积背景色需单独容器描述。
- **装饰边/特殊形状**：若设计图中有顶部/底部**波浪、弧线、S 形边**等（如蓝色顶栏底部为波浪），在对应节点的 `notes` 中必须写明，例如：`底部波浪/弧线，实现时用大圆角、CSS 渐变、伪元素或普通占位图块，禁止 SVG，勿简化为纯色矩形`，避免生成实现时漏掉。
...
6. **图标与资源**：设计图中的图标/头像是否在 HTML 中用真实存在的 `img` 切图、CSS 或普通元素体现，避免实现时变成占位块；不得使用 SVG，不得引用不存在的资源路径。
...
- `meta viewport` 使用 `width=device-width, initial-scale=1`，根容器按设计图实际宽度设置 `max-width`，布局、字号、颜色、圆角、内边距等按设计图实现。
