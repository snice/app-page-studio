...
const STORAGE_KEY = 'app-page-studio-exporter-settings-v2';
const MAX_PAGES = 20;
const MAX_SLICES_PER_PAGE = 120;

figma.showUI(__html__, { width: 380, height: 670, themeColors: true });
...
figma.ui.onmessage = async (message) => {
  try {
    ...
    if (message.type === 'scan') {
      ...
      figma.ui.postMessage({ type: 'scan-result', requestId: message.requestId, pages });
      return;
    }
    if (message.type === 'export') {
      const payload = await exportPayload(message.options || {});
      figma.ui.postMessage({ type: 'export-done', payload });
    }
  } catch (error) {
...
function getPageNodes(options = {}) {
  ...
  if (scope === 'current-page') {
    return figma.currentPage.children.filter(isPageNode).slice(0, MAX_PAGES);
  }
...
function postProgress(done, total, label) {
  figma.ui.postMessage({ type: 'export-progress', done, total, label });
}
...
async function exportPayload(options = {}) {
  ...
  return {
    source: {
      ...
      pageId: figma.currentPage && figma.currentPage.id ? figma.currentPage.id : null,
