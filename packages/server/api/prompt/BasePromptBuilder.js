...
### 图片资源处理
在生成代码时，请按以下规则处理图片资源：
1. **忽略 HTML 中的图片**：HTML 设计稿中的图片（img 标签、background-image 等）仅作为布局参考，**不要**在代码中引用或复制这些图片文件
2. **优先使用切图标注**：若页面提供了切图标注，将切图文件复制到 \`${g.assetsDir}\`，并在代码中使用切图替代对应区域的 HTML/CSS 还原
3. **无切图时使用占位**：对于没有切图标注的图片区域，使用普通布局背景（如纯色块、渐变色块）作为占位，保持布局结构不变即可
